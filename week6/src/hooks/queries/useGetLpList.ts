...
function useGetLpList({ cursor, search, order, limit }: PaginationDto) {
  return useQuery({
    ...
    // 컴포넌트가 창에 포커스 들어오는 경우도 재요청 X
    // 5분동안 기존 데이터를 그대로 활용해서 네트워크 요청을 줄임
    staleTime: 1000 * 50 * 5, // 5분

    // 사용되지 않는 (비활성 상태)인 쿼리 데이터가 캐시에 남아있는 시간
    // staleTime이 지나고 데이터가 신선하지 않더라도, 일정 시간 동안 메모리에 보관
    // 그 이후에 해당 쿼리가 전혀 사용되지 않으면 gcTime이 지난 후에 제가 (garbage collection)
    // 예) 10분동안 사용되지 않으면 해당 캐시 데이터가 삭제, 다시 요청시 새 데이터를 받아오에
    ...
    // 컴포넌트가 렌더링 될 때 빈 데이터 구조를 미리 제공 -> 로딩 전에도 안전하게 UI 구성할 수 있게
